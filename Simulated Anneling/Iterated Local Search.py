...
def perturbation(route):
    ...
    for k in range(10):
        for i in range(1000):
            ...
            first_position = random.randint(1, len(new_s) - 2)
            second_position = random.randint(1, len(new_s) - 2)
            new_s2 = swap_positions(new_s.copy(), first_position, second_position)
...
def main():
    ...
    max_visited = 50000  # Maximum number of neighborhood solutions visited
    ...
    for i in range(max_iteration):
        new_s = local_search(best_path, max_visited)
        s0 = new_s.copy()
        ...
        if fo(pert_s) < fo(best_path):
            best_path = pert_s.copy()

        X.append(i), y.append(round(fo(best_path), 3))
