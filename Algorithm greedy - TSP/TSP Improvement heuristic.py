...
def local_search(teste):
    """ heuristica de melhoramento - busca na vizinhança"""
    border = 4
    min_fo = fo(teste)

    for k in range(len(teste) - border):
        # print(k, "pivo:", teste[k])
        so = teste.copy()
        # print("Inicial:", teste)
        for i in range(border):
            sl = swapPositions(teste, k, i + 1 + k)
            teste = so.copy()

            # encontrar a fitness de cada sl para saber se a solução viznha melhora ou não o resultado
            custo_associado = fo(sl)
            #print(i, " ", custo_associado, ":", sl,)
            if custo_associado < min_fo:
                min_fo = custo_associado
                min_path = sl
    return min_fo, min_path
...
def main():
    ...
    for k in file_names:
        ...
        best_solution = []
        for r in range(1):  # range(len(seed)):
            best, order_tsp = problem(r, seed[r], k)

            # implementação da heuristica de melhoramento da solução encontrada
            min_fo, min_path = local_search(order_tsp)
            print("Melhor solução inicial: ", round(best,3))
            while abs(min_fo - best) > 0.5:
                best = min_fo
                min_fo, min_path = local_search(min_path)
                order_tsp = min_path
            best = min_fo
            print("Após a heuristica: ", round(min_fo, 3))
            best_solution.append(round(float(best), 2))
            plot_bestfit(order_tsp, r, k)

        print("\nMelhor valor obtido em cada uma das %d rodadas: %0.3f" % (r+1, best_solution[0]))
        print("Média dos valores: ", sum(best_solution) / len(best_solution))
        print("Desvio Padrão: ", np.std(best_solution))
        print("\nMenor valor obtido entre todas as %d rodadas: %0.3f" % (r+1, min(best_solution)))
        t_final = time.time()
